...
public class MemberDaoImpl implements IMemberDao{
	...
	public int insertMember(MemberVO memVo) {
		...
		try {
			...
			logger.info("PreparedStatement 객체 생성 완료");
			pstmt.setString(1, memVo.getMem_id());
			pstmt.setString(2, memVo.getMem_name());
			pstmt.setString(3, memVo.getMem_tel());
			pstmt.setString(4, memVo.getMem_addr());
			cnt = pstmt.executeUpdate();
			logger.info("실행된 SQL 문 ==> " + sql);
			logger.info("사용된 데이터 [ " +  memVo.getMem_id() + ", " + memVo.getMem_name() 
			...
			logger.info("PreparedStatement 닫기 완료");
			if(conn!=null)try {conn.close();} catch (Exception e2) {}
			logger.info("Connection 닫기 완료");
		}
		return cnt;
	...
	public int updateMember(MemberVO memVo) {
		...
		try {
			...
			cnt = pstmt.executeUpdate();
			logger.info("실행된 SQL 문 ==> " + sql);
			logger.info("사용된 데이터 [ " +  memVo.getMem_id() +" ," +memVo.getMem_name()+" ,"+memVo.getMem_tel()
	...
	public List<MemberVO> getAllMember() {
		...
		try {
			...
			logger.info("쿼리 실행 및  ResultSet 객체 생성 완료");
			//?
			while(rs.next()){
