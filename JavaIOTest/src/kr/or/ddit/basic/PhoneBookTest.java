...
public class PhoneBookTest {
	...
	public static void main(String[] args) {
		...
		try {
			...
			oin = new ObjectInputStream(new BufferedInputStream(
					new FileInputStream("./memObj.dat")));

			phoneBook = (HashMap<String, Phone>) oin.readObject();
		...
		while (true) {
			...
			case 0:
				...
				savePhone(phoneBook);
				System.out.println("저장하고 프로그램을 종료합니다.");
				System.exit(0);
			}
		}
	...
	private static void savePhone(HashMap<String, Phone> phoneBook) {
	...
	/*
	 * 문제) 이름, 주소, 나이, 전화번호를 멤버변수로 갖는 Phone클래스를 만들고 Map을 이용하여 전화번호 정보를 관리하는
	 * 프로그램을 작성하시오.
	 * 
	 * - 이 프로그램에는 전화번호를 등록, 수정, 삭제, 검색, 전체 출력하는 기능이 있다. - 삭제와 검색 기능은 '이름'을 입력 받아
	 * 처리한다. - (Map의 구조는 key값으로 그 사람의 '이름'을 사용하고, value값으로는 'Phone클래스의 인스턴스'로
	 * 한다.)
	 * 
	 * 실행예시) 다음 메뉴를 선택하세요. 1. 전화번호 등록 2. 전화번호 수정 3. 전화번호 삭제 4. 전화번호 검색 5. 전화번호
	 * 전체출력 0. 프로그램 종료 --------------- 번호입력> 1
	 * 
	 * 새롭게 등록할 전화번호 정보를 입력하세요 이름 >> 홍길동 전화번호 >> 010-1111-1111 나이 >> 30 주소 >> 대전시
	 * 중구 대흥동
	 * 
	 * '홍길동' 전화번호 등록 완료!!
	 * 
	 * 
	 * - 추가조건 _ 6. 1) 전화번호 저장을 추가하고 구현한다 저장파일명ㅇ느 phoneData.dat 2) 프로그램이 처음시작될때
	 * 저장된 파일이 있으면 그 데이터를 읽어와서 map에 셋팅한다. 3) 프로그램을 종료할때 Map의 데이터가 변경되었거나 추가 또는
	 * 삭제 되면 새로 저장한 후 종료
	 */
...
class Phone implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 545615513812334159L;
	String name;
	String age;
	String addr;
	String tel;
